locals {
  ...
  vms_metadata_public_image_html = {
    "user-data" : "#cloud-config\nusers:\n  - name: ${var.vms_ssh_user}\n    groups: sudo\n    shell: /bin/bash\n    sudo: 'ALL=(ALL) NOPASSWD:ALL'\n    ssh_authorized_keys:\n      - ${var.vms_ssh_root_key}\n#!/bin/bash\n echo '<html><body><img src='https://storage.yandexcloud.net/${yandex_storage_bucket.bucket.bucket}/${yandex_storage_object.content.key}'></body></html>' > /var/www/html/index.html"
  }
